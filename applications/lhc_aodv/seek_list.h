...
typedef struct seek_list {
    ...
    struct ip_data *ipd;
    unsigned char flags;		/* The flags we are using for resending the RREQ */
    int reqs;
    int ttl;
...
seek_list_t *seek_list_insert(struct in_addr dest_addr, unsigned int dest_seqno,
			      int ttl, unsigned char flags, struct ip_data *ipd);
