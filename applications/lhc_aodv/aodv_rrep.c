...
RREP *NS_CLASS rrep_create(unsigned char flags,
...
{
	unsigned int rrep_lifetime, rrep_seqno, rrep_new_hcnt;
	unsigned char pre_repair_hcnt = 0, pre_repair_flags = 0;
	...
	while ((rreplen - extlen) > RREP_SIZE)
	{
		...
		case RREP_INET_DEST_EXT:
			if (ext->length == sizeof(unsigned int))
			{
...
int rrep_add_hello_ext(RREP *rrep, int offset, unsigned int interval)
{
	AODV_ext *ext;
