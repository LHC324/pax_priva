...
long NS_CLASS hello_jitter()
{
	...
	return (long) (((float) Random::integer(RAND_MAX + 1) / RAND_MAX - 0.5)
		       * JITTER_INTERVAL);
#else
	srand((unsigned)time(NULL)); //也可以用rt-thread的HAL_GetTick()或其他系统时钟

	return (long)((((float)(rand() % 32768)) / RAND_MAX - 0.5) * JITTER_INTERVAL);
...
{
    unsigned int hello_seqno, timeout, hello_interval = HELLO_INTERVAL;
    unsigned char state, flags = 0;
